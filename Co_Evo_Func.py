...
def summarize_repeatability(df):
    ''' Summarize the repeatability scores of different samples
    repeatbility score = the frequency of replicates with the same maximum increasing member
    '''

    df = df[['sample', 'fold_increase']]
    df['max'] = df['fold_increase'].apply(np.argmax)
    df_sum = df.groupby(['sample', 'max'])['max'].count().groupby('sample').apply(pmax).reset_index()
    return df_sum['max'].values
...
def plot_trajectory(community, ax, cf, cm,
    ...
    for i, ide in enumerate(set(d['ident'][:ides])):
        data = d[d['ident'] == ide][d['total'] > 10]
        for st in sps:
            if alpha == 'changing':
                al = (i + 1) / len(set(d['ident'][:ides])) - 0.01
            colr = cm[st]
            ax.errorbar(y=data[st], x=data["Generation"],
                        yerr=data.apply(lambda x: np.sqrt(x[st] * (1 - x[st]) / x['total']),
                                        axis=1).values, fmt='|',  ecolor=colr, alpha=al)

            data.plot(y=st, x='Generation', ax=ax, legend=False,
                      colors=colr, alpha=al)
            data.plot.scatter(y=st, x='Generation', ax=ax, legend=False,
                              colors=colr, alpha=al, s=40)
    ax.set_ylim(0, 1.)
    ax.set_xticks(xtick);
